...
export default function LogoMakerPage() {
  ...
  return (
    <div className="min-h-screen bg-[#00050a] rounded-2xl text-white overflow-x-hidden selection:bg-indigo-500/30 font-sans">
      ...
      <main className="mx-auto p-4 md:p-8 grid grid-cols-1 lg:grid-cols-12 gap-8">
        <aside className="lg:col-span-4 space-y-6">
           <div className="bg-[#0a0c10] rounded-[32px] p-6 border border-white/10 space-y-6">
              <div className="space-y-4">
                 ...
                 <textarea
                   ...
                   className="w-full min-h-[160px] bg-white/5 rounded-2xl p-5 text-white placeholder:text-gray-600 outline-none border border-transparent focus:border-indigo-500/30 transition-all resize-none leading-relaxed"
                 />
              </div>
           ...
           <div className="bg-gradient-to-br from-indigo-900/10 to-transparent rounded-3xl p-6 border border-white/5">
              <h3 className="text-sm font-bold mb-3 flex items-center gap-2">
                 <Sparkles size={16} className="text-indigo-400" /> نصائح لنتائج أفضل
              </h3>
        ...
        <section className="lg:col-span-8 space-y-6">
           <div className="relative min-h-[500px] flex flex-col">
              <AnimatePresence mode="wait">
                 {selectedImage ? (
                    ...
                    >
                       <img src={selectedImage.url} className="max-h-[500px] w-auto rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)] relative z-10" />
                       <div className="absolute inset-x-0 bottom-10 flex justify-center opacity-0 group-hover:opacity-100 transition-opacity z-20">
                          <Button 
                             onClick={() => downloadImage(selectedImage.url, `logo-${selectedImage.id}.png`)}
                             className="rounded-full bg-indigo-600 hover:bg-indigo-700 h-10 px-8 font-bold"
                          >
                             <Download size={16} className="ml-2" /> تحميل الشعار بدقة عالية
                          </Button>
                       </div>
                    ...
                    <div className="flex-1 rounded-[40px] border-2 border-dashed border-white/5 flex flex-col items-center justify-center p-12 text-center group translate-y-0 hover:-translate-y-2 transition-transform duration-500">
                       <div className="w-24 h-24 rounded-full bg-indigo-500/10 flex items-center justify-center mb-6 group-hover:scale-110 transition-all border border-indigo-500/20 shadow-xl shadow-indigo-500/5">
                          <LayoutTemplate size={40} className="text-indigo-400" />
                       </div>
                       <h3 className="text-2xl font-bold mb-3">هويتك البصرية تبدأ هنا</h3>
           ...
           {history.length > 0 && (
             <div className="space-y-4">
                <h4 className="text-sm font-bold text-gray-400 px-2 flex items-center gap-2">
                   <History size={14} /> أرشيف التصميمات
                </h4>
                <div className="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-9 gap-3">
                   {history.map(img => (
                      <motion.div 
                        ...
                        className={clsx(
                           "aspect-square rounded-2xl cursor-pointer overflow-hidden border transition-all",
                           selectedImage?.id === img.id ? "border-indigo-500 ring-4 ring-indigo-500/20 scale-105" : "border-white/5 hover:border-white/20"
                        )}
                      >
