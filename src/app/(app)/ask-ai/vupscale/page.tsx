...
import { 
  ...
  Loader2, ArrowRight, Zap, FileVideo, ShieldCheck, X, Trash2
} from "lucide-react";
        ...
        <aside className="lg:col-span-4 space-y-6">
          <div className="bg-[#0a0c10] rounded-[32px] p-6 border border-white/10 space-y-6 shadow-2xl">
             <div className="space-y-4">
                <label className="text-xs font-bold text-gray-400 block text-right">الفيديو الأصلي (Low Quality)</label>
                <div className="aspect-video rounded-2xl border-2 border-dashed border-white/10 flex flex-col items-center justify-center cursor-pointer overflow-hidden bg-white/5 group/upload" onClick={() => document.getElementById('file-v-u')?.click()}>
                   {previewUrl ? (
                     <>
                   ...
                   ) : <Upload className="text-gray-700 group-hover/upload:text-blue-400" size={32} />}
                </div>
             ...
             <div className="p-4 bg-blue-500/10 rounded-2xl border border-blue-500/20 flex gap-3 shadow-inner">
                <ShieldCheck className="text-blue-400 shrink-0" size={20} />
                <p className="text-[10px] text-gray-400 text-right">سيقوم نظامنا بإعادة بناء البكسلات المفقودة وتوضيح الحواف باستخدام الذكاء الاصطناعي الفائق.</p>
             </div>
              ...
              <AnimatePresence mode="wait">
                 {selectedResult ? (
                    <motion.div key="res" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="relative z-10 w-full flex flex-col items-center">
                       ...
                       <video src={selectedResult.url} controls autoPlay loop className="max-h-[550px] w-auto rounded-[30px] border border-white/5 shadow-3xl transition-transform duration-500 hover:scale-[1.01]" />
                       <div className="mt-8 flex gap-3">
                          <Button onClick={() => window.open(selectedResult.url)} className="rounded-full bg-blue-600 hover:bg-blue-700 font-bold h-10 px-8 transition-all hover:scale-105"><Download className="mr-2 h-4 w-4" /> تحميل النسخة المحسنة</Button>
                    ...
                    <div className="text-center group">
                       <FileVideo size={80} className="text-blue-500/10 mb-6 mx-auto group-hover:scale-110 transition-transform duration-500 animate-pulse" />
                       <h3 className="text-2xl font-bold text-white mb-2">وضوح فائق الدقة</h3>
                       <p className="text-sm text-gray-500">ارفع الفيديو وسنقوم بتحويله إلى دقة احترافية.</p>
                    </div>
                 )}
              </AnimatePresence>
           </div>
                ...
                <div className="flex gap-4 overflow-x-auto pb-4 px-2 custom-scrollbar">
                   {history.map(h => (
                      <div key={h.id} className="relative group flex-shrink-0" onClick={() => setSelectedResult(h)}>
                        <div 
                           className={clsx("w-32 aspect-video rounded-2xl cursor-pointer border-2 transition-all overflow-hidden shadow-lg", selectedResult?.id === h.id ? "border-blue-500 scale-110 opacity-100" : "border-white/5 opacity-50 hover:opacity-100")}
                        >
