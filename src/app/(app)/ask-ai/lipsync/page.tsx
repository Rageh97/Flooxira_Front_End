...
export default function LipSyncPage() {
  ...
  const loadHistory = async () => {
    ...
    try {
      ...
      // Filter for lipsync operations only
      const mappedHistory: any[] = response.history
        .filter((item: AIHistoryItem) => (item.options as any)?.operation === 'lipsync')
          ...
          <div className="h-full overflow-y-auto scrollbar-hide p-6 space-y-5">
            <div className="space-y-4">
             <div className="space-y-4">
                ...
                <div className="relative aspect-video rounded-2xl border-2 border-dashed border-white/10 flex flex-col items-center justify-center cursor-pointer overflow-hidden bg-white/5 group" onClick={() => document.getElementById('file-v-l')?.click()}>
                   {videoPreview ? (
                     <>
                        <video src={videoPreview} className="w-full h-full object-cover opacity-50 group-hover:opacity-30 transition-opacity" />
                   ...
                   ) : <Upload className="text-gray-700 group-hover:text-green-400" size={32} />}
                </div>
