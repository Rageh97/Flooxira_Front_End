...
import { 
  ...
  Loader2, ArrowRight, Image as ImageIcon, Zap, Wand2 
} from "lucide-react";
        ...
        <aside className="lg:col-span-4 space-y-6">
          <div className="bg-[#0a0c10] rounded-[32px] p-6 border border-white/10 space-y-6">
            <div className="space-y-4">
               <label className="text-xs font-bold text-gray-400">صورة المنتج</label>
               <div className="aspect-video rounded-xl border-2 border-dashed border-white/10 flex items-center justify-center cursor-pointer overflow-hidden hover:border-blue-500/30 transition-all" onClick={() => document.getElementById('file-p')?.click()}>
                  {previewUrl ? <img src={previewUrl} className="w-full h-full object-contain" /> : <Package className="text-gray-600" size={32} />}
               </div>
            ...
            <div className="space-y-4">
               <label className="text-xs font-bold text-gray-400">وصف البيئة المحيطة</label>
               <textarea value={prompt} onChange={e => setPrompt(e.target.value)} placeholder="مثال: على طاولة خشبية فخمة، إضاءة سينمائية من الخلف، خلفية ضبابية لمطبخ عصري..." className="w-full h-32 bg-white/5 border-none rounded-2xl p-4 text-sm placeholder:text-gray-600 outline-none resize-none" />
            </div>
        ...
        <section className="lg:col-span-8 space-y-6">
           <div className="min-h-[500px] rounded-[40px] bg-[#0a0c10] border border-white/10 flex items-center justify-center p-8 relative overflow-hidden group">
              <AnimatePresence mode="wait">
                {selectedResult ? (
                  <motion.div key="res" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="relative z-10">
                ...
                ) : isProcessing ? <AILoader /> : <div className="text-gray-700 flex flex-col items-center"><Package size={80} className="mb-4 opacity-10" /><p>اعرض منتجاتك بجودة استديو احترافي</p></div>}
              </AnimatePresence>
           </div>
           
           {history.length > 0 && (
             <div className="grid grid-cols-6 gap-3">
               {history.map(h => <img key={h.id} src={h.url} onClick={() => setSelectedResult(h)} className={clsx("aspect-square rounded-xl cursor-pointer border-2 transition-all", selectedResult?.id === h.id ? "border-blue-500" : "border-transparent opacity-50")} />)}
             </div>
           )}
        </section>
      </main>
